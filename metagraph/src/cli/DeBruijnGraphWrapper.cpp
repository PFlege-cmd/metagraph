...
long DeBruijnGraphWrapper::retrieveAnchorCoordinates(std::string anchor_sequence,
    ...
    //std::cout << "anchor coords Size: " << anchor_coords.size() << std::endl;
     for (unsigned long k = 0; k < anchor_coords.size(); ++k) {
         ...
         std::cout << "Curr genome" << genome_name << std::endl;
         std::cout << "Kmer genome " << curr_genome << std::endl;
...
std::vector<GraphWrapper::genomeCoordinateTriple> DeBruijnGraphWrapper::get_kmer_coordinates(std::string_view& kmer) {
    ...
    const double discovery_fraction = 0.699999999999996;
    const double presence_fraction = 0.0;
    return this->get_graph()->get_kmer_coordinates(kmer, num_top_labels, discovery_fraction, presence_fraction);
}

size_t DeBruijnGraphWrapper::get_K() const {
    return this->get_graph()->get_graph().get_k();
}
